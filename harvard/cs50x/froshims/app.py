...
#:  4.1.- The request method behaves differently depending on HTML "forms" method.
...
@app.route("/")
def index():
    #: We can pass to the index.html file, a parameter that jinja can use to load all the sports
...
def register():
    ...
    for sport in request.form.getlist("sport"):
        if sport not in SPORTS:
            return render_template("failure.html")
    if (len(request.form.getlist("sport")) == 0):
        return render_template("failure.html")
