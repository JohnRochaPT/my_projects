...
def index():
    ...
    for stock in stocks:
        ...
        currentStockValue = currentStockValue + \
            (currentStockInfo['price'] * stock['numberOfShares'])
...
def buy():
    ...
    if request.method == 'GET':
        ...
        if not symbol:
            return apology(f'Stock symbol is empty', 400)

        # Check to see that the symbol is a valid stock symbol
        price = lookup(symbol)
        if not price:
            return apology(f'Stock symbol {symbol} does not exists', 400)
        ...
        try:
            ...
            if quantity <= 0:
                return apology('Negative amount is invalid', 400)
        except:
            return apology('Amount is invalid', 400)
        pass
    ...
    print(
        f'Attempting to purchase [{quantity}] shares of [{symbol}] at a price of [{price['price']}]')
...
def register():
    ...
    try:
        ...
        if len(rows) == 0:
            return apology('Unable to get the new row from the database', 400)
        session['user_id'] = rows[0]['id']
        print('Session stated and now redirecting to "/"')
        return redirect('/')
    except:
        return apology('Unable to insert into the database', 400)
...
def sell():
    ...
    if request.method == 'POST':
        ...
        howMany = request.form.get('shares')
        if not stockToSell or not howMany:
            return apology('No symbol or number or shares', 400)
        ...
        print(f'Type of howMany is [{type(howMany)}]')

        # Attempt to turn howMany to an integer
        try:
        ...
        if howMany > int(canSell[0]['totalShares']):
            return apology('Selling more shares than in stock', 400)
        else:
            print('Enough shares')
        ...
        print(
            f'Selling for a total of [{howMany}] [{stockToSell}] shares for a total of [{totalSale}]')
